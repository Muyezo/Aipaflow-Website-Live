...
export function useBlog() {
  ...
  const getBlogPosts = useCallback(async (options?: {
    ...
    try {
      let query = supabase
        .from('blog_posts')
        .select('*, author:profiles!blog_posts_author_id_fkey(*)', { count: 'exact' });
      ...
      return { data: transformedData as BlogPost[], count: count || 0 };
    } catch (error) {
  ...
  const getBlogPost = useCallback(async (slug: string) => {
    ...
    try {
      const { data, error } = await supabase
        .from('blog_posts')
        .select('*, author:profiles!blog_posts_author_id_fkey(*)')
        .eq('slug', slug)
        .single();
